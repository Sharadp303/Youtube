...
import { youtubeSearchQuery } from "../utils/constants";
...
const Header = () => {
  ...
  const getSearchSuggestions = async () => {
    const data = await fetch(youtubeSearchQuery + searchQuery);
  ...
  return (
    <div className="grid grid-flow-col p-5 m-2 shadow-lg">
      ...
      <div className="col-span-10">
        ...
        {showSuggestions && (
          <div className="absolute z-10  bg-white shadow-lg rounded-lg border border-gray-100  ">
            <ul>
              {suggestions.map((item) => {
                return (
                  <li
                    key={item + "num"}
                    className="py-2 px-3 w-[29rem] shadow-sm hover:bg-slate-100"
                  >
